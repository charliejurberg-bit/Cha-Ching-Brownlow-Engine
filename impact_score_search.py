`"""
Impact Score Formula Search
Tests 200 random combinations of stats + coefficients to find the best Impact Score formula.
Saves every result to predictions/impact_score_results.csv as it goes.
"""
...
RESULTS_PATH = "predictions/impact_score_results.csv"
N_TRIALS = 200
N_SPLITS = 5
RANDOM_SEED = 42
...
# Stats pool: label -> column name in DataFrame
STAT_POOL = {
    'Disposals':              'Disposals',
    ...
    'Score_Involvements':     'Score_Involvements',22
    'Tackles':                'Tackles',
    'Marks':                  'Marks',
    'Handballs':              'Handballs',
    'Kicks':                  'Kicks',
...
# Dynamic per-season max H&A round: finals are string-labeled (QF/EF/SF/PF/GF) -> NaN
before = len(stats)
...
# ── Static relative features (no Impact_Score) ───────────────
...
w = (0.85 ** (max_s - df_clean['Season'])).values.flatten()
groups = df_clean['Season'].values.flatten().astype(int)

gkf = GroupKFold(n_splits=N_SPLITS)
X_static = df_clean[STATIC_FEATURES].values  # precomputed static part
...
for trial_idx in range(start_trial, N_TRIALS):
    n_stats = random.randint(3, 8)
    selected = random.sample(STAT_LABELS, n_stats)
    formula = {s: round(random.uniform(0.5, 4.0), 2) for s in selected}
    ...
    result = {
        ...
        'formula': formula_str,
        **{f'coef_{s}': formula.get(s, 0.0) for s in STAT_LABELS},
    }
...
print(f"\n{'='*65}")
print(f"SEARCH COMPLETE — {N_TRIALS} combinations tested")
print(f"{'='*65}")
